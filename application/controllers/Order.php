...
//---------------order2 entires-------------------
            ...
            //-----------order as guest -----------
                ...
                //---------get cart data----------------
                $cartInfo= $this->session->userdata('cart_data');
    ...
    public function check_pincode()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run()== true) {
                ...
                $order_data= $this->db->get()->row();
                if ($order_data->total_amount < 250) {
                    //----calculate shipping charges-----
                    $this->db->select('*');
                    $this->db->from('tbl_pincode');
                    $this->db->like('pincode', $pincode);
                    ...
                    if (!empty($pin_data)) {
                        ...
                        if ($i==1) {
                            $shipping = $pin_data->shippingcharge;
                            $final_amount = $order_data->total_amount + $shipping - $order_data->p_discount;
